...
- Autenticação Segura: Suporte a autenticação AES para tipos de SysCon: CXR, CXRF e SW. Leitura de Logs de Erros: Executa comandos como "ERRLOG" para coletar até 32 códigos de erro, filtra e exibe descrições baseadas em um banco de dados JSON (em português e inglês).

- Ciclos de Teste: Realiza power cycles automáticos (bringup/shutdown) para gerar novos logs de erros.

- Controle do Console: Comandos para ligar (bringup), desligar (shutdown) e limpar logs de erros.

- Exportação de Dados: Salva erros em TXT/CSV, cópia para clipboard e exibição em janela popup.

- Interface Amigável: Suporte a idiomas (Pt-Br/En), barra de progresso e logos rotativos.

- Logging Detalhado: Registra todas as operações em ps3syscon.log para debug.
